...
TEE_Result TA_OpenSessionEntryPoint(uint32_t param_types, TEE_Param params[4], void **sess_ctx)
{
	...
	if (data == NULL || data->wallet_handle == NULL || data->storageID == NULL || data->write_cache == NULL || data->aes_encrypt_op == NULL || data->aes_decrypt_op == NULL || data->hmac_op1_internal == NULL || data->hmac_op2_internal == NULL || data->hmac_op1 == NULL || data->hmac_op2 == NULL || data->sha_op_internal == NULL || data->sha_op1 == NULL || data->sha_op2 == NULL || data->sha_op3 == NULL || data->sha_op4 == NULL || data->current_wallet == NULL)
		return TEE_ERROR_OUT_OF_MEMORY;
...
static TEE_Result sha256_update(Session_data *session_data, uint32_t param_types, TEE_Param params[4])
{
	...
	TEE_MemMove((uint8_t *)(params[0].memref.buffer), data, length);
	TEE_Free(data);
...
static TEE_Result sha256_final(Session_data *session_data, uint32_t param_types, TEE_Param params[4])
{
	...
	data = TEE_Malloc(written_hash_length, 0);
	if (!data)
		return TEE_ERROR_OUT_OF_MEMORY;
	TEE_MemMove(data, (uint8_t *)(params[0].memref.buffer), written_hash_length);
...
static TEE_Result hmac_sha256(Session_data *session_data, uint32_t param_types, TEE_Param params[4])
{
	...
	/* Some sanity checks */
	if (param_types != exp_param_types || params[0].memref.buffer == NULL || params[1].memref.buffer == NULL || params[2].memref.buffer == NULL)
		return TEE_ERROR_BAD_PARAMETERS;
...
static TEE_Result open_wallet_storage(Session_data *session_data, uint32_t param_types, TEE_Param params[4])
{
	...
	/* Check if the storage was already open or created */
	if (session_data->is_storage_open == true || session_data->is_storage_created == false)
		return TEE_ERROR_BAD_STATE;
...
static TEE_Result close_wallet_storage(Session_data *session_data, uint32_t param_types, TEE_Param params[4])
{
	...
	/* Check if the storage was already open or created */
	if (session_data->is_storage_open == false || session_data->is_storage_created == false)
		return TEE_ERROR_BAD_STATE;
...
static TEE_Result flush_wallet_storage_internal(Session_data *session_data, NonVolatileReturn *nv_error)
{
	...
	*nv_error = NV_NO_ERROR;
	...
	if (session_data->is_storage_created == false || session_data->is_storage_open == false)
	{
...
static TEE_Result ecdsa_sign_test(Session_data *session_data, uint32_t param_types, TEE_Param params[4])
{
	...
	/* Some sanity checks */
	if (param_types != exp_param_types || params[0].memref.buffer == NULL || params[1].memref.buffer == NULL || params[2].memref.buffer == NULL || params[3].memref.buffer == NULL)
		return TEE_ERROR_BAD_PARAMETERS;
...
static TEE_Result generate_deterministic256_test(Session_data *session_data, uint32_t param_types, TEE_Param params[4])
{
	...
	/* Some sanity checks */
	if (param_types != exp_param_types || params[0].memref.buffer == NULL || params[1].memref.buffer == NULL || params[3].memref.buffer == NULL)
		return TEE_ERROR_BAD_PARAMETERS;
...
static WalletErrors get_private_key_internal(Session_data *session_data, uint8_t *out, AddressHandle ah)
{
	...
	if ((ah == 0) || (ah > (session_data->current_wallet->encrypted.num_addresses)) || (ah == BAD_ADDRESS_HANDLE))
	{
...
static TEE_Result change_encryption_key(Session_data *session_data, uint32_t param_types, TEE_Param params[4])
{
	...
	/* Some sanity checks */
	if (param_types != exp_param_types) // remove NULL checking for buffer
		return TEE_ERROR_BAD_PARAMETERS;
...
/* Here should be an generic error but as it is not available ... */
	...
	{
		if ((session_data->current_wallet->unencrypted.version == VERSION_UNENCRYPTED) || (session_data->current_wallet->unencrypted.version == VERSION_IS_ENCRYPTED))
...
static TEE_Result get_wallet_info(Session_data *session_data, uint32_t param_types, TEE_Param params[4])
{
	...
	if (param_types != exp_param_types || params[1].memref.buffer == NULL || params[2].memref.buffer == NULL)
		return TEE_ERROR_BAD_PARAMETERS;
...
static TEE_Result new_wallet(Session_data *session_data, uint32_t param_types, TEE_Param params[4])
{
	...
	if (exp_param_types != param_types || params[0].memref.buffer == NULL || params[3].memref.buffer == NULL)
		return TEE_ERROR_BAD_PARAMETERS;
